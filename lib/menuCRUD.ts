...
export async function createCategory(
  input: Pick<Category, "key" | "label" | "icon">,
) {
...
export async function updateCategory(
  key: string,
  patch: Partial<Omit<Category, "key">>,
) {
...
export async function deleteCategory(key: string) {
  if (!key) return;
  // Evitar orfandad: chequeá si hay items con esa categoría
  const q = query(collection(db, "items"), where("category", "==", key));
  const snap = await getDocs(q);
  if (!snap.empty) {
    throw new Error(
      `No se puede borrar la categoría "${key}" porque tiene ${snap.size} ítems.`,
    );
  }
...
// Opcional: mover todos los items de una categoría a otra (batch)
export async function bulkMoveItems(oldKey: string, newKey: string) {
  ...
  snap.forEach((d) =>
    batch.update(d.ref, { category: newKey, updatedAt: serverTimestamp() }),
  );
  await batch.commit();
}
...
export async function setDailyMenu(data: DailyMenu) {
  await setDoc(
    doc(db, "dailyMenu", "current"),
    {
      ...data,
      updatedAt: serverTimestamp(),
      createdAt: serverTimestamp(), // si no existe, Firestore lo crea
    },
    { merge: true },
  );
}
